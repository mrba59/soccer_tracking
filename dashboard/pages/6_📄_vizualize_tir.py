# dashboard/pages/6_📄_liste_des_tirs.py
...
st.set_page_config(
    page_title="Soccer Stats - Liste des tirs",
    page_icon="📄",
    layout="wide",
)

st.title("📄 Liste des tirs")

# ---- FICHIERS ----
PASSES_JSON = STATS_DIR / "passes.json"                # même source que page 5
VIDEO_2D_FULL = DATA_DIR / "full_game_2D_passe.mp4"    # même vidéo de référence
...
def load_events(path: Path) -> pd.DataFrame:
    ...
    expected_cols = [
        ...
        "tireur", "speed", "longueur", "nb_player_elimine",
    ...
    # Coercitions sûres
    num_cols = ["start", "end", "team", "id", "passeur", "receveur", "tireur",
    ...
    # Map équipes passer/receveur (si présent) -> icône
    def team_to_icon(v):
        if pd.isna(v):
            return "?"
        try:
...
datas = events[events["type"] == "tir"].copy()

# ---- AgGrid ----
display_cols = [
    "start", "end", "id", "speed", "tireur", "team",
    "second_start", "second_duration",
]
display_cols = [c for c in display_cols if c in datas.columns]

if len(display_cols) == 0:
    st.warning("Aucune colonne affichable trouvée pour les tirs.")
else:
    ...
    if len(selected_rows) == 0:
        st.info("Sélectionne un tir dans le tableau pour afficher la vidéo et les détails.")
    else:
        row = selected_rows[0]
        ...
        with col2:
            ...
            st.write(f"Vitesse : {_val('speed')}")

            tireur = _val("tireur")
            team = _val("team")
            st.write(f"Tireur : n°{tireur} • Équipe: {team}")
